...
如果你用 Cloudflare 路由把 `app.example.com/api/*` 也转到 Worker，则可以保持为空，前端会请求相对路径 `/api/*`。
...
```text
app.example.com       -> Cloudflare Pages 自定义域名
*.sites.example.com   -> Cloudflare Worker route
example.com           -> 官网或跳转到 app
```
