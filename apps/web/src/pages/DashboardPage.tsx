...
const steps = [
  ...
  { id: 3 as const, label: "发布版本", icon: Rocket },
];
  ...
  async function createProject(event: React.FormEvent) {
    ...
    try {
      ...
      window.history.replaceState(
        {},
        "",
        `/studio?project=${encodeURIComponent(created.id)}`,
      );
    } catch (cause) {
  ...
  async function deploy() {
    ...
    try {
      ...
      if (uploadSession.status === "blocked")
        throw new Error("服务端检查未通过，请修正资源后重试");
      const deployed =
      ...
      if (deployed.status === "blocked")
        throw new Error("版本生成被安全检查阻止，请根据诊断修正资源");
      setResult(deployed);
      setSite({
        ...site,
  ...
  if (!authReady || restoring)
    return (
      <StudioLoading
        account={account}
        active="create"
        label={restoring ? "正在恢复创建进度" : "正在读取账号"}
        ...
        <div className={STUDIO_CONTENT_SHELL_CLASS}>
          ...
          <div className={STUDIO_MAIN_CLASS}>
            ...
            >
              {steps.map((item) => {
                ...
                return (
                  <li
                    className={cn(
                      "flex min-w-0 items-center gap-2 border-b-2 px-2 py-4 sm:px-4",
                      active
            ...
            {step === 1 ? (
              ...
              >
                <label className="grid w-full gap-2 text-sm font-medium text-zinc-200">
                  项目名称
                  <input
                    className="h-11 w-full rounded-md border border-white/20 bg-black px-3 text-white outline-none transition-colors placeholder:text-zinc-600 focus:border-white/50"
                    disabled={creating}
                    maxLength={80}
                ...
                <p className="mt-4 text-sm leading-6 text-zinc-500">
                  项目创建后默认为仅自己可见。生成版本后，可以为它设置公开地址。
                </p>
                <button
                  className="mt-6 inline-flex h-11 cursor-pointer items-center gap-2 rounded-md bg-white px-5 text-sm font-semibold text-black transition-colors hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
              ...
              <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_18rem]">
                <div className={`${STUDIO_PANEL_CLASS} p-5 sm:p-6`}>
                  ...
                  {prepared ? (
                    <div className="mt-5">
                      <div className="grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-3">
                        {[
                          ["文件", `${prepared.scan.fileCount} 个`],
                          ["大小", formatBytes(prepared.scan.totalBytes)],
                          ["入口", prepared.scan.entrypoint ?? "未找到"],
                        ].map(([label, value]) => (
                      ...
                      {prepared.sourceRoot ? (
                        <p className="mt-3 text-sm text-zinc-400">
                          将使用 {prepared.sourceRoot}/ 作为发布目录
                        </p>
                      ) : null}
                      {prepared.scan.issues.length > 0 ? (
                        <div className="mt-4 grid gap-2">
                          {prepared.scan.issues.map((issue) => (
                            <p
                              className={cn(
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <button
                      className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-md border border-white/15 px-4 text-sm text-zinc-300 hover:bg-white/5"
                    ...
                    <button
                      className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-md bg-white px-4 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-50"
                      disabled={!prepared || blocked || analyzing}
                      onClick={() => setStep(3)}
                      type="button"
                    >
                      确认资源
                ...
                <aside className={`${STUDIO_PANEL_CLASS} h-fit p-5`}>
                  <p className="text-xs text-zinc-500">私人项目已创建</p>
                  ...
                  <p className="mt-5 border-t border-white/10 pt-4 text-sm leading-6 text-zinc-500">
                    现在退出也不会丢失。版本生成后，可在项目详情中绑定或切换公开地址。
                  </p>
                </aside>
              </div>
            ) : null}

            {step === 3 && site && prepared ? (
                ...
                {result ? (
                  <div className="py-4 text-center">
                    ...
                    <p className="mt-2 text-sm text-zinc-400">
                      ...
                      {formatBytes(result.totalBytes)}。当前只有你可以访问。
                    </p>
                    <div className="mt-6 flex flex-wrap justify-center gap-3">
                      <button
                        className="inline-flex h-10 cursor-pointer items-center rounded-md bg-white px-4 text-sm font-semibold text-black"
                        onClick={() =>
                          onNavigate(
                            `${STUDIO_PROJECTS_PATH}/${site.id}?tab=publishing`,
                          )
                        }
                        type="button"
                      >
                      ...
                      <button
                        className="inline-flex h-10 cursor-pointer items-center rounded-md border border-white/15 px-4 text-sm text-zinc-300 hover:bg-white/5"
                        onClick={() =>
                          onNavigate(`${STUDIO_PROJECTS_PATH}/${site.id}`)
                        }
                        type="button"
                      >
                        查看项目概览
                      ...
                      <button
                        className="inline-flex h-10 cursor-pointer items-center rounded-md border border-transparent px-4 text-sm text-zinc-500 hover:text-white"
                        onClick={() => onNavigate(STUDIO_PROJECTS_PATH)}
                        type="button"
                      >
                        返回项目列表
                  ...
                  <>
                    <div className="flex items-start gap-3">
                      <FileCheck2 className="mt-0.5 h-5 w-5 text-zinc-400" />
                      <div>
                        <h2 className="text-base font-semibold">
                          生成版本确认
                        </h2>
                        <p className="mt-1 text-sm text-zinc-500">
                          {site.name} · {prepared.scan.fileCount} 个文件 ·{" "}
                          {formatBytes(prepared.scan.totalBytes)} ·{" "}
                          {getRiskLabel(prepared.scan.riskLevel)}
                        </p>
                      </div>
                    </div>
                    <div className="mt-5 grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-2">
                      <div className="bg-black p-4">
                        <p className="text-xs text-zinc-500">可见范围</p>
                        <p className="mt-2 text-sm text-white">仅自己可见</p>
                      </div>
                      <div className="bg-black p-4">
                        <p className="text-xs text-zinc-500">入口文件</p>
                        <p className="mt-2 text-sm text-white">
                          {prepared.scan.entrypoint}
                        </p>
                      </div>
                    </div>
                    <div className="mt-6 flex flex-wrap gap-3">
                      ...
                      <button
                        className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-md bg-white px-5 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-50"
                        disabled={deploying}
                        onClick={deploy}
                        type="button"
                      >
            ...
            <p className="mt-5 text-xs text-zinc-600">
              单站点上限 {formatBytes(plan.quotas.site.maxSiteBytes)}，最多{" "}
              {plan.quotas.deployment.maxFiles} 个文件，单文件上限{" "}
              {formatBytes(plan.quotas.deployment.maxFileBytes)}
            </p>
          </div>
        </div>
      </section>
