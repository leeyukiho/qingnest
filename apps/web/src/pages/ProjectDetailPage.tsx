...
export function ProjectDetailPage({ account, authReady, onNavigate, session, siteId }: {
  ...
  const refresh = () => Promise.all([getProject(siteId), listPublicSlots()]).then(([data, publicSlots]) => { setProject(data); setName(data.name); setSlots(publicSlots); });
  ...
  async function saveName(event: React.FormEvent) {
    ...
    try { const updated = await updateProject(siteId, { name }); setProject(updated); showToast("项目名称已保存", "success"); }
  ...
  useEffect(() => {
    let active = true;
    setPrepared(null);
    if (files.length === 0) { setChecking(false); return () => { active = false; }; }
    ...
    prepareProjectDeployment(files, account?.plan ?? "free")
      .then((result) => {
        if (!active) return;
        setPrepared(result);
        showToast(hasBlockingScanIssues(result.scan) ? "项目文件检查未通过" : `检查完成：${result.scan.fileCount} 个文件，${formatBytes(result.scan.totalBytes)}`, hasBlockingScanIssues(result.scan) ? "error" : "success");
      })
      .catch((cause) => { if (active) showToast(cause instanceof Error ? cause.message : "项目文件检查失败", "error"); })
  ...
  async function createAndBindSlot(event: React.FormEvent) {
    ...
    if (!validation.ok) return showToast(validation.reason ?? "请输入可用的公开地址", "error");
    ...
    } catch (cause) { showToast(cause instanceof Error ? cause.message : "公开地址创建失败", "error"); }
    finally { setPublishingBusy(false); }
  }

  async function bindSlot(slot: PublicSlot) {
    if (slot.siteId === siteId) return;
    if (slot.siteId && !window.confirm(`切换后，${slot.hostname} 的访客将看到当前项目。确认切换吗？`)) return;
    setPublishingBusy(true);
    try { await switchPublicSlot(slot.id, siteId); await refresh(); showToast("公开地址已切换到当前项目", "success"); }
    catch (cause) { showToast(cause instanceof Error ? cause.message : "切换失败", "error"); }
    finally { setPublishingBusy(false); }
  }

  async function unbindSlot(slot: PublicSlot) {
    if (!window.confirm(`解绑后，${slot.hostname} 将暂时无法访问。确认解绑吗？`)) return;
    setPublishingBusy(true);
    try { await switchPublicSlot(slot.id, null); await refresh(); showToast("公开地址已解绑", "success"); }
    catch (cause) { showToast(cause instanceof Error ? cause.message : "解绑失败", "error"); }
    finally { setPublishingBusy(false); }
  }
  ...
  async function previewPrivateProject() {
    ...
    } catch (cause) { previewWindow?.close(); showToast(cause instanceof Error ? cause.message : "无法创建私人预览", "error"); }
    finally { setPublishingBusy(false); }
  }
          ...
          <div className={STUDIO_MAIN_CLASS}>
            ...
            {project && tab === "publishing" ? <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_18rem]">
              <div className={`${STUDIO_PANEL_CLASS} p-5 sm:p-6`}>
                ...
                {slots.length < plan.quotas.user.maxPublicSites ? <form className="mt-6 border-t border-white/10 pt-5" onSubmit={createAndBindSlot}><label className="grid gap-2 text-sm font-medium text-zinc-300">创建新的公开地址<div className="grid grid-cols-[minmax(0,1fr)_auto] overflow-hidden rounded-md border border-white/20 focus-within:border-white/50"><input className="h-11 min-w-0 bg-black px-3 text-white outline-none placeholder:text-zinc-600" disabled={!canPublish || publishingBusy} onChange={(event) => setSubdomain(event.target.value.toLowerCase())} placeholder="my-project" value={subdomain} /><span className="inline-flex items-center border-l border-white/15 px-3 text-sm text-zinc-500">.985201314.xyz</span></div></label><button className="mt-4 inline-flex h-10 cursor-pointer items-center gap-2 rounded-md bg-white px-4 text-sm font-semibold text-black disabled:opacity-50" disabled={!canPublish || publishingBusy || !subdomain.trim()} type="submit">{publishingBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Globe2 className="h-4 w-4" />}创建并公开</button></form> : <p className="mt-6 border-t border-white/10 pt-5 text-sm text-zinc-500">公开站点额度已用完。升级套餐或购买额外公开地址后，可以同时公开更多项目。</p>}
              </div>
              ...
              {files.length > 0 ? <div className="mt-5 rounded-md border border-white/10 bg-white/[0.025] p-4">
                ...
                {prepared ? <>
                  <div className="mt-4 grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-3">
                    <div className="bg-black p-3"><p className="text-xs text-zinc-500">文件数量</p><p className="mt-1 text-sm font-semibold text-white">{prepared.scan.fileCount} 个</p></div>
                    <div className="bg-black p-3"><p className="text-xs text-zinc-500">项目大小</p><p className="mt-1 text-sm font-semibold text-white">{formatBytes(prepared.scan.totalBytes)}</p></div>
                    <div className="bg-black p-3"><p className="text-xs text-zinc-500">入口文件</p><p className="mt-1 truncate text-sm font-semibold text-white">{prepared.scan.entrypoint ?? "未找到"}</p></div>
                  </div>
                  <div className="mt-4">
                    <p className="flex items-center gap-2 text-xs font-medium text-zinc-400"><FolderTree className="h-3.5 w-3.5" />项目结构</p>
                    <div className="mt-2 grid gap-1.5">{prepared.scan.files.slice(0, 8).map((item) => <p className="flex min-w-0 items-center gap-2 text-xs text-zinc-500" key={item.path}><File className="h-3 w-3 shrink-0" /><span className="truncate">{item.path}</span><span className="ml-auto shrink-0">{formatBytes(item.size)}</span></p>)}</div>
                    {prepared.scan.fileCount > 8 ? <p className="mt-2 text-xs text-zinc-600">另有 {prepared.scan.fileCount - 8} 个文件</p> : null}
                  </div>
